...
static l_noret error_expected (LexState *ls, int token) {
  vmkX_syntaxerror(ls,
      vmkO_pushfstring(ls->L, "%s expected", vmkX_token2str(ls, token)));
}
...
void vmkY_checklimit (FuncState *fs, int v, int l, const char *what) {
...
#define check_condition(ls,c,msg)	{ if (!(c)) vmkX_syntaxerror(ls, msg); }
...
static void check_match (LexState *ls, int what, int who, int where) {
  if (l_unlikely(!testnext(ls, what))) {
    if (where == ls->linenumber)  /* all in the same line? */
      ...
      vmkX_syntaxerror(ls, vmkO_pushfstring(ls->L,
             "%s expected (to close %s at line %d)",
              vmkX_token2str(ls, what), vmkX_token2str(ls, who), where));
    }
  }
}
...
static short registerlocalvar (LexState *ls, FuncState *fs,
  ...
  vmkC_objbarrier(ls->L, f, varname);
...
#define new_localvarliteral(ls,v) \
    new_localvar(ls,  \
      vmkX_newstring(ls, "" v, (sizeof(v)/sizeof(char)) - 1));
...
** fn.
*/
lu_byte vmkY_nvarstack (FuncState *fs) {
...
static void check_readonly (LexState *ls, expdesc *e) {
  ...
  if (varname) {
    const char *msg = vmkO_pushfstring(ls->L,
       "attempt to assign to const variable '%s'", getstr(varname));
    vmkK_semerror(ls, msg);  /* error */
  }
}
...
static void adjustlocalvars (LexState *ls, int nvars) {
  FuncState *fs = ls->fs;
  int reglevel = vmkY_nvarstack(fs);
  int i;
  for (i = 0; i < nvars; i++) {
    int vidx = fs->nactvar++;
...
static Upvaldesc *allocupvalue (FuncState *fs) {
  ...
  vmkY_checklimit(fs, fs->nups + 1, MAXUPVAL, "upvalues");
  vmkM_growvector(fs->ls->L, f->upvalues, fs->nups, f->sizeupvalues,
...
static int newupvalue (FuncState *fs, TString *name, expdesc *v) {
  ...
  if (v->k == VLOCAL) {
    ...
    vmk_assert(eqstr(name, getlocalvardesc(prev, v->u.var.vidx)->vd.name));
  }
  else {
    up->instack = 0;
    up->idx = cast_byte(v->u.info);
    up->kind = prev->f->upvalues[v->u.info].kind;
    vmk_assert(eqstr(name, prev->f->upvalues[v->u.info].name));
  }
  up->name = name;
  vmkC_objbarrier(fs->ls->L, fs->f, name);
  return fs->nups - 1;
}
...
static void singlevar (LexState *ls, expdesc *var) {
  ...
  if (var->k == VVOID) {  /* global name? */
    ...
    vmk_assert(var->k != VVOID);  /* this one must exist */
    vmkK_exp2anyregup(fs, var);  /* but could be a constant */
    codestring(&key, varname);  /* key is variable name */
    vmkK_indexed(fs, var, &key);  /* env[varname] */
  }
}
...
static void adjust_assign (LexState *ls, int nvars, int nexps, expdesc *e) {
  ...
  if (hasmultret(e->k)) {  /* last expression has multiple returns? */
    ...
    if (extra < 0)
      extra = 0;
    vmkK_setreturns(fs, e, extra);  /* last exp. provides the difference */
  }
  else {
    if (e->k != VVOID)  /* at least one expression? */
      vmkK_exp2nextreg(fs, e);  /* close last expression */
    if (needed > 0)  /* missing values? */
      vmkK_nil(fs, fs->freereg, needed);  /* complete with nils */
  }
  if (needed > 0)
    vmkK_reserveregs(fs, needed);  /* registers for extra values */
  else  /* adding 'needed' is actually a subtraction */
    fs->freereg = cast_byte(fs->freereg + needed);  /* remove extra values */
}


#define enterlevel(ls)	vmkE_incCstack(ls->L)
...
static void closegoto (LexState *ls, int g, Labeldesc *label, int bup) {
  ...
  vmk_assert(eqstr(gt->name, label->name));
  ...
  vmkK_patchlist(ls->fs, gt->pc, label->pc);  /* goto jumps to label */
...
static int newlabelentry (LexState *ls, Labellist *l, TString *name,
                          int line, int pc) {
  int n = l->n;
  vmkM_growvector(ls->L, l->arr, n, l->size,
...
static int newgotoentry (LexState *ls, TString *name, int line) {
  FuncState *fs = ls->fs;
  int pc = vmkK_jump(fs);  /* create jump */
  vmkK_codeABC(fs, OP_CLOSE, 0, 1, 0);  /* spaceholder, marked as dead */
...
static void createlabel (LexState *ls, TString *name, int line, int last) {
  ...
  int l = newlabelentry(ls, ll, name, line, vmkK_getlabel(fs));
...
static void enterblock (FuncState *fs, BlockCnt *bl, lu_byte isloop) {
  ...
  bl->previous = fs->bl;
  fs->bl = bl;
  vmk_assert(fs->freereg == vmkY_nvarstack(fs));
}
...
static l_noret undefgoto (LexState *ls, Labeldesc *gt) {
  ...
  msg = vmkO_pushfstring(ls->L, msg, getstr(gt->name), gt->line);
  /* breaks are checked when created, cannot be undefined */
  vmk_assert(!eqstr(gt->name, vmkS_newliteral(ls->L, "break")));
  vmkK_semerror(ls, msg);
}
...
static void leaveblock (FuncState *fs) {
  ...
  vmk_assert(bl->nactvar == fs->nactvar);  /* back to level on entry */
  if (bl->isloop == 2)  /* has to fix pending breaks? */
    createlabel(ls, vmkS_newliteral(ls->L, "break"), 0, 0);
  solvegotos(fs, bl);
...
static Proto *addprototype (LexState *ls) {
  ...
  f->p[fs->np++] = clp = vmkF_newproto(L);
  vmkC_objbarrier(L, f, clp);
  return clp;
}
...
static void codeclosure (LexState *ls, expdesc *v) {
  ...
  init_exp(v, VRELOC, vmkK_codeABx(fs, OP_CLOSURE, 0, fs->np - 1));
  vmkK_exp2nextreg(fs, v);  /* fix it at the last register */
}
...
static void open_func (LexState *ls, FuncState *fs, BlockCnt *bl) {
  ...
  fs->bl = NULL;
  f->source = ls->source;
  vmkC_objbarrier(L, f, f->source);
  f->maxstacksize = 2;  /* registers 0/1 are always valid */
  fs->kcache = vmkH_new(L);  /* create table for fn */
...
static void close_func (LexState *ls) {
  ...
  FuncState *fs = ls->fs;
  Proto *f = fs->f;
  vmkK_ret(fs, vmkY_nvarstack(fs), 0);  /* final return */
  leaveblock(fs);
  vmk_assert(fs->bl == NULL);
  vmkK_finish(fs);
  vmkM_shrinkvector(L, f->code, f->sizecode, fs->pc, Instruction);
  vmkM_shrinkvector(L, f->lineinfo, f->sizelineinfo, fs->pc, ls_byte);
  vmkM_shrinkvector(L, f->abslineinfo, f->sizeabslineinfo,
                       fs->nabslineinfo, AbsLineInfo);
  vmkM_shrinkvector(L, f->k, f->sizek, fs->nk, TValue);
  vmkM_shrinkvector(L, f->p, f->sizep, fs->np, Proto *);
  vmkM_shrinkvector(L, f->locvars, f->sizelocvars, fs->ndebugvars, LocVar);
  vmkM_shrinkvector(L, f->upvalues, f->sizeupvalues, fs->nups, Upvaldesc);
  ls->fs = fs->prev;
...
static void fieldsel (LexState *ls, expdesc *v) {
  ...
  FuncState *fs = ls->fs;
  expdesc key;
  vmkK_exp2anyregup(fs, v);
  vmkX_next(ls);  /* skip the dot or colon */
  codename(ls, &key);
  vmkK_indexed(fs, v, &key);
}


static void yindex (LexState *ls, expdesc *v) {
  /* index -> '[' expr ']' */
  vmkX_next(ls);  /* skip the '[' */
  expr(ls, v);
  vmkK_exp2val(ls->fs, v);
  checknext(ls, ']');
}
...
static void recfield (LexState *ls, ConsControl *cc) {
  ...
  if (ls->t.token == TK_NAME) {
    vmkY_checklimit(fs, cc->nh, INT_MAX / 2, "items in a constructor");
    codename(ls, &key);
  }
  else  /* ls->t.token == '[' */
    yindex(ls, &key);
  ...
  checknext(ls, '=');
  tab = *cc->t;
  vmkK_indexed(fs, &tab, &key);
  expr(ls, &val);
  vmkK_storevar(fs, &tab, &val);
...
static void closelistfield (FuncState *fs, ConsControl *cc) {
  ...
  vmkK_exp2nextreg(fs, &cc->v);
  cc->v.k = VVOID;
  if (cc->tostore >= cc->maxtostore) {
    vmkK_setlist(fs, cc->t->u.info, cc->na, cc->tostore);  /* flush */
    cc->na += cc->tostore;
...
static void lastlistfield (FuncState *fs, ConsControl *cc) {
  ...
  if (hasmultret(cc->v.k)) {
    vmkK_setmultret(fs, &cc->v);
    vmkK_setlist(fs, cc->t->u.info, cc->na, VMK_MULTRET);
    cc->na--;  /* do not count last expression (unknown number of elements) */
  }
  else {
    if (cc->v.k != VVOID)
      vmkK_exp2nextreg(fs, &cc->v);
    vmkK_setlist(fs, cc->t->u.info, cc->na, cc->tostore);
  }
  cc->na += cc->tostore;
}
...
static void field (LexState *ls, ConsControl *cc) {
  ...
  switch(ls->t.token) {
    case TK_NAME: {  /* may be 'listfield' or 'recfield' */
      if (vmkX_lookahead(ls) != '=')  /* expression? */
        listfield(ls, cc);
      else
        recfield(ls, cc);
      break;
...
static void constructor (LexState *ls, expdesc *t) {
  ...
  int pc = vmkK_codevABCk(fs, OP_NEWTABLE, 0, 0, 0, 0);
  ConsControl cc;
  vmkK_code(fs, 0);  /* space for extra arg. */
  ...
  do {
    vmk_assert(cc.v.k == VVOID || cc.tostore > 0);
    if (ls->t.token == /*{*/ '}') break;
  ...
  check_match(ls, /*{*/ '}', '{' /*}*/, line);
  lastlistfield(fs, &cc);
  vmkK_settablesize(fs, pc, t->u.info, cc.na, cc.nh);
}
...
static void setvararg (FuncState *fs, int nparams) {
  ...
  vmkK_codeABC(fs, OP_VARARGPREP, nparams, 0, 0);
}
...
static void parlist (LexState *ls) {
  ...
  if (ls->t.token != ')') {  /* is 'parlist' not empty? */
    do {
      switch (ls->t.token) {
        ...
        default: vmkX_syntaxerror(ls, "<name> or '...' expected");
      }
  ...
  vmkK_reserveregs(fs, fs->nactvar);  /* reserve registers for parameters */
}
...
static void funcargs (LexState *ls, expdesc *f) {
  ...
  switch (ls->t.token) {
    ...
    case TK_STRING: {  /* funcargs -> STRING */
      ...
      vmkX_next(ls);  /* must use 'seminfo' before 'next' */
      break;
    }
    default: {
  ...
  if (hasmultret(args.k))
    nparams = VMK_MULTRET;  /* open call */
  else {
    if (args.k != VVOID)
      vmkK_exp2nextreg(fs, &args);  /* close last argument */
    nparams = fs->freereg - (base+1);
  }
  init_exp(f, VCALL, vmkK_codeABC(fs, OP_CALL, base, nparams+1, 2));
  vmkK_fixline(fs, line);
  /* call removes fn and arguments and leaves one result (unless
     changed later) */
...
static void primaryexp (LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    ...
    default: {
      vmkX_syntaxerror(ls, "unexpected symbol");
    }
  }
}
...
static BinOpr subexpr (LexState *ls, expdesc *v, int limit) {
  ...
  if (uop != OPR_NOUNOPR) {  /* prefix (unary) operator? */
    int line = ls->linenumber;
    vmkX_next(ls);  /* skip operator */
    subexpr(ls, v, UNARY_PRIORITY);
    vmkK_prefix(ls->fs, uop, v, line);
  }
  else simpleexp(ls, v);
  ...
  while (op != OPR_NOBINOPR && priority[op].left > limit) {
    ...
    int line = ls->linenumber;
    vmkX_next(ls);  /* skip operator */
    vmkK_infix(ls->fs, op, v);
    /* read sub-expression with higher priority */
    nextop = subexpr(ls, &v2, priority[op].right);
    vmkK_posfix(ls->fs, op, v, &v2, line);
    op = nextop;
  }
  leavelevel(ls);
...
static void check_conflict (LexState *ls, struct LHS_assign *lh, expdesc *v) {
  ...
  if (conflict) {
    ...
    if (v->k == VLOCAL)
      ...
      vmkK_codeABC(fs, OP_GETUPVAL, extra, v->u.info, 0);
    vmkK_reserveregs(fs, 1);
  }
}
...
static void restassign (LexState *ls, struct LHS_assign *lh, int nvars) {
  ...
  if (testnext(ls, ',')) {  /* restassign -> ',' suffixedexp restassign */
    ...
    if (nexps != nvars)
      ...
      vmkK_setoneret(ls->fs, &e);  /* close last expression */
      vmkK_storevar(ls->fs, &lh->v, &e);
...
static void breakstat (LexState *ls, int line) {
  ...
  vmkX_syntaxerror(ls, "break outside loop");
 ok:
  bl->isloop = 2;  /* signal that block has pending breaks */
  vmkX_next(ls);  /* skip break */
  newgotoentry(ls, vmkS_newliteral(ls->L, "break"), line);
}
...
static void checkrepeated (LexState *ls, TString *name) {
  ...
  if (l_unlikely(lb != NULL)) {  /* already defined? */
    ...
    msg = vmkO_pushfstring(ls->L, msg, getstr(name), lb->line);
    vmkK_semerror(ls, msg);  /* error */
  }
}
...
static void whilestat (LexState *ls, int line) {
  ...
  int condexit;
  BlockCnt bl;
  vmkX_next(ls);  /* skip WHILE */
  whileinit = vmkK_getlabel(fs);
  condexit = cond(ls);
  enterblock(fs, &bl, 1);
  checknext(ls, TK_DO);
  block(ls);
  vmkK_jumpto(fs, whileinit);
  check_match(ls, TK_END, TK_WHILE, line);
  leaveblock(fs);
  vmkK_patchtohere(fs, condexit);  /* false conditions finish the loop */
}
...
static void repeatstat (LexState *ls, int line) {
  ...
  int condexit;
  FuncState *fs = ls->fs;
  int repeat_init = vmkK_getlabel(fs);
  BlockCnt bl1, bl2;
  enterblock(fs, &bl1, 1);  /* loop block */
  enterblock(fs, &bl2, 0);  /* scope block */
  vmkX_next(ls);  /* skip REPEAT */
  statlist(ls);
  check_match(ls, TK_UNTIL, TK_REPEAT, line);
  condexit = cond(ls);  /* read condition (inside scope block) */
  leaveblock(fs);  /* finish scope */
  if (bl2.upval) {  /* upvalues? */
    int exit = vmkK_jump(fs);  /* normal exit must jump over fix */
    vmkK_patchtohere(fs, condexit);  /* repetition must close upvalues */
    vmkK_codeABC(fs, OP_CLOSE, reglevel(fs, bl2.nactvar), 0, 0);
    condexit = vmkK_jump(fs);  /* repeat after closing upvalues */
    vmkK_patchtohere(fs, exit);  /* normal exit comes to here */
  }
  vmkK_patchlist(fs, condexit, repeat_init);  /* close the loop */
...
static void exp1 (LexState *ls) {
  expdesc e;
  expr(ls, &e);
  vmkK_exp2nextreg(ls->fs, &e);
  vmk_assert(e.k == VNONRELOC);
}


/*
** Fix for instruction at position 'pc' to jump to 'dest'.
** (Jump addresses are relative in Vmk). 'back' true means
** a back jump.
*/
...
static void forbody (LexState *ls, int base, int line, int nvars, int isgen) {
  ...
  int prep, endfor;
  checknext(ls, TK_DO);
  prep = vmkK_codeABx(fs, forprep[isgen], base, 0);
  fs->freereg--;  /* both 'forprep' remove one register from the stack */
  enterblock(fs, &bl, 0);  /* scope for declared variables */
  adjustlocalvars(ls, nvars);
  vmkK_reserveregs(fs, nvars);
  block(ls);
  leaveblock(fs);  /* end of scope for declared variables */
  fixforjump(fs, prep, vmkK_getlabel(fs), 0);
  if (isgen) {  /* generic for? */
    vmkK_codeABC(fs, OP_TFORCALL, base, 0, nvars);
    vmkK_fixline(fs, line);
  }
  endfor = vmkK_codeABx(fs, forloop[isgen], base, 0);
...
static void forlist (LexState *ls, TString *indexname) {
  ...
  FuncState *fs = ls->fs;
  expdesc e;
  int nvars = 4;  /* fn, state, closing, control */
  int line;
  int base = fs->freereg;
  /* create internal variables */
  new_localvarliteral(ls, "(for state)");  /* iterator fn */
  ...
  vmkK_checkstack(fs, 2);  /* extra space to call iterator */
...
static void forstat (LexState *ls, int line) {
  ...
  vmkX_next(ls);  /* skip 'for' */
  varname = str_checkname(ls);  /* first variable name */
  switch (ls->t.token) {
    case '=': fornum(ls, varname, line); break;
    case ',': case TK_IN: forlist(ls, varname); break;
    default: vmkX_syntaxerror(ls, "'=' or 'in' expected");
  }
...
static void test_then_block (LexState *ls, int *escapelist) {
  ...
  FuncState *fs = ls->fs;
  int condtrue;
  vmkX_next(ls);  /* skip IF or ELSEIF */
  condtrue = cond(ls);  /* read condition */
  checknext(ls, TK_THEN);
  block(ls);  /* 'then' part */
  if (ls->t.token == TK_ELSE ||
      ls->t.token == TK_ELSEIF)  /* followed by 'else'/'elseif'? */
    vmkK_concat(fs, escapelist, vmkK_jump(fs));  /* must jump over it */
  vmkK_patchtohere(fs, condtrue);
}
...
static void ifstat (LexState *ls, int line) {
  ...
  vmkK_patchtohere(fs, escapelist);  /* patch escape list to 'if' end */
}
...
static lu_byte getlocalattribute (LexState *ls) {
  ...
  if (testnext(ls, '<')) {
    ...
    if (strcmp(attr, "const") == 0)
      ...
      vmkK_semerror(ls,
        vmkO_pushfstring(ls->L, "unknown attribute '%s'", attr));
  }
...
static void checktoclose (FuncState *fs, int level) {
  if (level != -1) {  /* is there a to-be-closed variable? */
    marktobeclosed(fs);
    vmkK_codeABC(fs, OP_TBC, reglevel(fs, level), 0, 0);
  }
}
...
static void localstat (LexState *ls) {
  ...
  if (nvars == nexps &&  /* no adjustments? */
      ...
      vmkK_exp2const(fs, &e, &var->k)) {  /* compile-time constant? */
...
static void funcstat (LexState *ls, int line) {
  ...
  check_readonly(ls, &v);
  vmkK_storevar(ls->fs, &v, &b);
  vmkK_fixline(ls->fs, line);  /* definition "happens" in the first line */
}
...
static void retstat (LexState *ls) {
  ...
  int first = vmkY_nvarstack(fs);  /* first slot to be returned */
  if (block_follow(ls, 1) || ls->t.token == ';')
    nret = 0;  /* return no values */
  else {
    nret = explist(ls, &e);  /* optional return values */
    if (hasmultret(e.k)) {
      vmkK_setmultret(fs, &e);
      if (e.k == VCALL && nret == 1 && !fs->bl->insidetbc) {  /* tail call? */
        SET_OPCODE(getinstruction(fs,&e), OP_TAILCALL);
        vmk_assert(GETARG_A(getinstruction(fs,&e)) == vmkY_nvarstack(fs));
      }
      nret = VMK_MULTRET;  /* return all values */
    }
    else {
      if (nret == 1)  /* only one single value? */
        first = vmkK_exp2anyreg(fs, &e);  /* can use original slot */
      else {  /* values must go to the top of the stack */
        vmkK_exp2nextreg(fs, &e);
        vmk_assert(nret == fs->freereg - first);
      }
    }
  }
  vmkK_ret(fs, first, nret);
...
static void statement (LexState *ls) {
  ...
  switch (ls->t.token) {
    ...
    case TK_DBCOLON: {  /* stat -> label */
      vmkX_next(ls);  /* skip double colon */
      labelstat(ls, str_checkname(ls), line);
      break;
    }
    case TK_RETURN: {  /* stat -> retstat */
      vmkX_next(ls);  /* skip RETURN */
      retstat(ls);
      break;
    }
  ...
  vmk_assert(ls->fs->f->maxstacksize >= ls->fs->freereg &&
             ls->fs->freereg >= vmkY_nvarstack(ls->fs));
  ls->fs->freereg = vmkY_nvarstack(ls->fs);  /* free registers */
  leavelevel(ls);
}
...
static void mainfunc (LexState *ls, FuncState *fs) {
  ...
  env->kind = VDKREG;
  env->name = ls->envn;
  vmkC_objbarrier(ls->L, fs->f, env->name);
  vmkX_next(ls);  /* read first token */
...
LClosure *vmkY_parser (vmk_State *L, ZIO *z, Mbuffer *buff,
                       Dyndata *dyd, const char *name, int firstchar) {
  LexState lexstate;
  FuncState funcstate;
  LClosure *cl = vmkF_newLclosure(L, 1);  /* create main closure */
  setclLvalue2s(L, L->top.p, cl);  /* anchor it (to avoid being collected) */
  vmkD_inctop(L);
  lexstate.h = vmkH_new(L);  /* create table for scanner */
  sethvalue2s(L, L->top.p, lexstate.h);  /* anchor it */
  vmkD_inctop(L);
  funcstate.f = cl->p = vmkF_newproto(L);
  vmkC_objbarrier(L, cl, cl->p);
  funcstate.f->source = vmkS_new(L, name);  /* create and anchor TString */
  vmkC_objbarrier(L, funcstate.f, funcstate.f->source);
  lexstate.buff = buff;
  lexstate.dyd = dyd;
  dyd->actvar.n = dyd->gt.n = dyd->label.n = 0;
  vmkX_setinput(L, &lexstate, z, funcstate.f->source, firstchar);
  mainfunc(&lexstate, &funcstate);
  vmk_assert(!funcstate.prev && funcstate.nups == 1 && !lexstate.fs);
  /* all scopes should be correctly finished */
  vmk_assert(dyd->actvar.n == 0 && dyd->gt.n == 0 && dyd->label.n == 0);
