...
#define makewhite(g,x)	\
  (x->marked = cast_byte((x->marked & ~maskcolors) | vmkC_white(g)))
...
static void atomic (vmk_State *L);
static void entersweep (vmk_State *L);
...
static l_mem objsize (GCObject *o) {
  ...
  switch (o->tt) {
    ...
    case VMK_VPROTO: {
      res = vmkF_protosize(gco2p(o));
      break;
    }
    case VMK_VTHREAD: {
      res = vmkE_threadsize(gco2th(o));
      break;
    }
    case VMK_VSHRSTR: {
      TString *ts = gco2ts(o);
    ...
    case VMK_VLNGSTR: {
      TString *ts = gco2ts(o);
      res = vmkS_sizelngstr(ts->u.lnglen, ts->shrlen);
      break;
    }
    case VMK_VUPVAL: {
      res = sizeof(UpVal);
...
static GCObject **getgclist (GCObject *o) {
  switch (o->tt) {
    case VMK_VTABLE: return &gco2t(o)->gclist;
    case VMK_VLCL: return &gco2lcl(o)->gclist;
    case VMK_VCCL: return &gco2ccl(o)->gclist;
    case VMK_VTHREAD: return &gco2th(o)->gclist;
    case VMK_VPROTO: return &gco2p(o)->gclist;
    case VMK_VUSERDATA: {
      Udata *u = gco2u(o);
      vmk_assert(u->nuvalue > 0);
      return &u->gclist;
    }
    default: vmk_assert(0); return 0;
  }
}
...
static void linkgclist_ (GCObject *o, GCObject **pnext, GCObject **list) {
  vmk_assert(!isgray(o));  /* cannot be in a gray list */
  *pnext = *list;
  *list = o;
...
static int iscleared (global_State *g, const GCObject *o) {
  ...
  else if (novariant(o->tt) == VMK_TSTRING) {
...
** white from dead.)
*/
void vmkC_barrier_ (vmk_State *L, GCObject *o, GCObject *v) {
  global_State *g = G(L);
  vmk_assert(isblack(o) && iswhite(v) && !isdead(g, v) && !isdead(g, o));
  if (keepinvariant(g)) {  /* must keep invariant? */
    reallymarkobject(g, v);  /* restore invariant */
    if (isold(o)) {
      vmk_assert(!isold(v));  /* white object could not be old */
      setage(v, G_OLD0);  /* restore generational invariant */
    }
  }
  else {  /* sweep phase */
    vmk_assert(issweepphase(g));
...
*/
void vmkC_barrierback_ (vmk_State *L, GCObject *o) {
  global_State *g = G(L);
  vmk_assert(isblack(o) && !isdead(g, o));
  vmk_assert((g->gckind != KGC_GENMINOR)
...
void vmkC_fix (vmk_State *L, GCObject *o) {
  global_State *g = G(L);
  vmk_assert(g->allgc == o);  /* object must be 1st in 'allgc' list! */
...
*/
GCObject *vmkC_newobjdt (vmk_State *L, lu_byte tt, size_t sz, size_t offset) {
  global_State *g = G(L);
  char *p = cast_charp(vmkM_newobject(L, novariant(tt), sz));
  GCObject *o = cast(GCObject *, p + offset);
  o->marked = vmkC_white(g);
  o->tt = tt;
  o->next = g->allgc;
  g->allgc = o;
  return o;
...
*/
GCObject *vmkC_newobj (vmk_State *L, lu_byte tt, size_t sz) {
  return vmkC_newobjdt(L, tt, sz, 0);
}
...
static void reallymarkobject (global_State *g, GCObject *o) {
  ...
  switch (o->tt) {
    ...
    case VMK_VLCL: case VMK_VCCL: case VMK_VTABLE:
    case VMK_VTHREAD: case VMK_VPROTO: {
      linkobjgclist(o, g->gray);  /* to be visited later */
      break;
    }
    default: vmk_assert(0); break;
  }
}
...
static void remarkupvals (global_State *g) {
  ...
  while ((thread = *p) != NULL) {
    if (!iswhite(thread) && thread->openupval != NULL)
      ...
      UpVal *uv;
      vmk_assert(!isold(thread) || thread->openupval == NULL);
      *p = thread->twups;  /* remove thread from the list */
      thread->twups = thread;  /* mark that it is out of list */
      for (uv = thread->openupval; uv != NULL; uv = uv->u.open.next) {
        vmk_assert(getage(uv) <= getage(thread));
        if (!iswhite(uv)) {  /* upvalue already visited? */
          vmk_assert(upisopen(uv) && isgray(uv));
...
/*
** Traverse a Vmk closure, marking its prototype and its upvalues.
...
*/
static l_mem traversethread (global_State *g, vmk_State *th) {
  UpVal *uv;
  StkId o = th->stack.p;
  ...
  vmk_assert(g->gcstate == GCSatomic ||
  ...
  if (g->gcstate == GCSatomic) {  /* final traversal? */
    if (!g->gcemergency)
      vmkD_shrinkstack(th); /* do not change stack in emergency cycle */
...
static l_mem propagatemark (global_State *g) {
  ...
  switch (o->tt) {
    case VMK_VTABLE: return traversetable(g, gco2t(o));
    case VMK_VUSERDATA: return traverseudata(g, gco2u(o));
    case VMK_VLCL: return traverseLclosure(g, gco2lcl(o));
    case VMK_VCCL: return traverseCclosure(g, gco2ccl(o));
    case VMK_VPROTO: return traverseproto(g, gco2p(o));
    case VMK_VTHREAD: return traversethread(g, gco2th(o));
    default: vmk_assert(0); return 0;
  }
}
  ...
  for (; l != f; l = gco2t(l)->gclist) {
    ...
    for (i = 0; i < asize; i++) {
      ...
      if (iscleared(g, o))  /* value was collected? */
        *getArrTag(h, i) = VMK_VEMPTY;  /* remove entry */
    }
    for (n = gnode(h, 0); n < limit; n++) {
...
static void freeupval (vmk_State *L, UpVal *uv) {
  if (upisopen(uv))
    vmkF_unlinkupval(uv);
  vmkM_free(L, uv);
}
...
static void freeobj (vmk_State *L, GCObject *o) {
  assert_code(l_mem newmem = gettotalbytes(G(L)) - objsize(o));
  switch (o->tt) {
    case VMK_VPROTO:
      vmkF_freeproto(L, gco2p(o));
      break;
    case VMK_VUPVAL:
    ...
    case VMK_VLCL: {
      LClosure *cl = gco2lcl(o);
      vmkM_freemem(L, cl, sizeLclosure(cl->nupvalues));
      break;
    }
    case VMK_VCCL: {
      CClosure *cl = gco2ccl(o);
      vmkM_freemem(L, cl, sizeCclosure(cl->nupvalues));
      break;
    }
    case VMK_VTABLE:
      vmkH_free(L, gco2t(o));
      break;
    case VMK_VTHREAD:
      vmkE_freethread(L, gco2th(o));
      break;
    case VMK_VUSERDATA: {
      Udata *u = gco2u(o);
      vmkM_freemem(L, o, sizeudata(u->nuvalue, u->len));
      break;
    }
    case VMK_VSHRSTR: {
      TString *ts = gco2ts(o);
      vmkS_remove(L, ts);  /* remove it from hash table */
      vmkM_freemem(L, ts, sizestrshr(cast_uint(ts->shrlen)));
      break;
    }
    case VMK_VLNGSTR: {
      TString *ts = gco2ts(o);
      ...
      vmkM_freemem(L, ts, vmkS_sizelngstr(ts->u.lnglen, ts->shrlen));
      break;
    }
    default: vmk_assert(0);
  }
  vmk_assert(gettotalbytes(G(L)) == newmem);
}
...
*/
static GCObject **sweeplist (vmk_State *L, GCObject **p, l_mem countin) {
  global_State *g = G(L);
  int ow = otherwhite(g);
  int white = vmkC_white(g);  /* current white */
...
*/
static GCObject **sweeptolive (vmk_State *L, GCObject **p) {
  GCObject **old = p;
  do {
    p = sweeplist(L, p, 1);
  } while (p == old);
...
*/
static void checkSizes (vmk_State *L, global_State *g) {
  if (!g->gcemergency) {
    if (g->strt.nuse < g->strt.size / 4)  /* string table too big? */
      vmkS_resize(L, g->strt.size / 2);
  }
}
...
static void dothecall (vmk_State *L, void *ud) {
  UNUSED(ud);
  vmkD_callnoyield(L, L->top.p - 2, 0);
}


static void GCTM (vmk_State *L) {
  global_State *g = G(L);
  const TValue *tm;
  TValue v;
  vmk_assert(!g->gcemergency);
  setgcovalue(L, &v, udata2finalize(g));
  tm = vmkT_gettmbyobj(L, &v, TM_GC);
  if (!notm(tm)) {  /* is there a finalizer? */
    ...
    status = vmkD_pcall(L, dothecall, NULL, savestack(L, L->top.p - 2), 0);
    L->ci->callstatus &= ~CIST_FIN;  /* not running a finalizer anymore */
    L->allowhook = oldah;  /* restore hooks */
    g->gcstp = oldgcstp;  /* restore state */
    if (l_unlikely(status != VMK_OK)) {  /* error while running __gc? */
      vmkE_warnerror(L, "__gc");
...
*/
static void callallpendingfinalizers (vmk_State *L) {
  global_State *g = G(L);
  while (g->tobefnz)
    GCTM(L);
}
...
*/
void vmkC_checkfinalizer (vmk_State *L, GCObject *o, Table *mt) {
  global_State *g = G(L);
...
*/
static void sweep2old (vmk_State *L, GCObject **p) {
  GCObject *curr;
  global_State *g = G(L);
  while ((curr = *p) != NULL) {
    if (iswhite(curr)) {  /* is 'curr' dead? */
      vmk_assert(isdead(g, curr));
      *p = curr->next;  /* remove 'curr' from list */
      freeobj(L, curr);  /* erase 'curr' */
    }
    else {  /* all surviving objects become old */
      setage(curr, G_OLD);
      if (curr->tt == VMK_VTHREAD) {  /* threads must be watched */
        vmk_State *th = gco2th(curr);
        linkgclist(th, g->grayagain);  /* insert into 'grayagain' list */
      }
      else if (curr->tt == VMK_VUPVAL && upisopen(gco2upv(curr)))
...
static GCObject **sweepgen (vmk_State *L, global_State *g, GCObject **p,
  ...
  l_mem addedold = 0;
  int white = vmkC_white(g);
  GCObject *curr;
  while ((curr = *p) != limit) {
    if (iswhite(curr)) {  /* is 'curr' dead? */
      vmk_assert(!isold(curr) && isdead(g, curr));
      *p = curr->next;  /* remove 'curr' from list */
      freeobj(L, curr);  /* erase 'curr' */
    }
      ...
      if (age == G_NEW) {  /* new objects go back to white */
        ...
        vmk_assert(age != G_OLD1);  /* advanced in 'markold' */
...
static GCObject **correctgraylist (GCObject **p) {
  ...
  while ((curr = *p) != NULL) {
    ...
    if (iswhite(curr))
      ...
      vmk_assert(isgray(curr));
      nw2black(curr);  /* make it black, for next barrier */
      setage(curr, G_TOUCHED2);
      goto remain;  /* keep it in the list and go to next element */
    }
    else if (curr->tt == VMK_VTHREAD) {
      vmk_assert(isgray(curr));
      goto remain;  /* keep non-white threads on the list */
    }
    else {  /* everything else is removed */
      vmk_assert(isold(curr));  /* young objects should be white here */
...
*/
static void finishgencycle (vmk_State *L, global_State *g) {
  correctgraylists(g);
  checkSizes(L, g);
...
*/
static void minor2inc (vmk_State *L, global_State *g, lu_byte kind) {
  ...
  vmkE_setdebt(g, applygcparam(g, STEPSIZE, 100));
}
...
*/
static void youngcollection (vmk_State *L, global_State *g) {
  l_mem addedold1 = 0;
  l_mem marked = g->GCmarked;  /* preserve 'g->GCmarked' */
  GCObject **psurvival;  /* to point to first non-dead survival object */
  GCObject *dummy;  /* dummy out parameter to 'sweepgen' */
  vmk_assert(g->gcstate == GCSpropagate);
...
*/
static void atomic2gen (vmk_State *L, global_State *g) {
  cleargraylists(g);
...
static void setminordebt (global_State *g) {
  vmkE_setdebt(g, applygcparam(g, MINORMUL, g->GCmajorminor));
}
...
** collection.
*/
static void entergen (vmk_State *L, global_State *g) {
  vmkC_runtilstate(L, GCSpause, 1);  /* prepare to start a new cycle */
  vmkC_runtilstate(L, GCSpropagate, 1);  /* start new cycle */
...
*/
void vmkC_changemode (vmk_State *L, int newmode) {
  global_State *g = G(L);
  ...
  if (newmode != g->gckind) {  /* does it need to change? */
    if (newmode == KGC_INC)  /* entering incremental mode? */
      ...
      vmk_assert(newmode == KGC_GENMINOR);
      entergen(L, g);
    }
  }
}
...
*/
static void fullgen (vmk_State *L, global_State *g) {
...
*/
static int checkmajorminor (vmk_State *L, global_State *g) {
...
** 'limit'.
*/
static void deletelist (vmk_State *L, GCObject *p, GCObject *limit) {
  while (p != limit) {
...
/*
** Call all finalizers of the objects in the given Vmk state, and
** then free all objects, except for the main thread.
*/
void vmkC_freeallobjects (vmk_State *L) {
  global_State *g = G(L);
  g->gcstp = GCSTPCLS;  /* no extra finalizers after here */
  vmkC_changemode(L, KGC_INC);
  separatetobefnz(g, 1);  /* separate all objects with finalizers */
  vmk_assert(g->finobj == NULL);
  callallpendingfinalizers(L);
  deletelist(L, g->allgc, obj2gco(g->mainthread));
  vmk_assert(g->finobj == NULL);  /* no new finalizers */
  deletelist(L, g->fixedgc, NULL);  /* collect fixed objects */
  vmk_assert(g->strt.nuse == 0);
}
...
static void atomic (vmk_State *L) {
  ...
  g->grayagain = NULL;
  vmk_assert(g->ephemeron == NULL && g->weak == NULL);
  vmk_assert(!iswhite(g->mainthread));
  g->gcstate = GCSatomic;
...
*/
static void sweepstep (vmk_State *L, global_State *g,
...
static l_mem singlestep (vmk_State *L, int fast) {
  global_State *g = G(L);
  l_mem stepresult;
  vmk_assert(!g->gcstopem);  /* collector is not reentrant */
...
** here is always true.)
*/
void vmkC_runtilstate (vmk_State *L, int state, int fast) {
  global_State *g = G(L);
  vmk_assert(g->gckind == KGC_INC);
...
*/
static void incstep (vmk_State *L, global_State *g) {
  ...
  if (g->gcstate == GCSpause)
    ...
    vmkE_setdebt(g, stepsize);
}


#if !defined(vmki_tracegc)
#define vmki_tracegc(L,f)		((void)0)
#endif
...
*/
void vmkC_step (vmk_State *L) {
  global_State *g = G(L);
  vmk_assert(!g->gcemergency);
  if (!gcrunning(g)) {  /* not running? */
    if (g->gcstp & GCSTPUSR)  /* stopped by the user? */
      vmkE_setdebt(g, 20000);
  }
  else {
    vmki_tracegc(L, 1);  /* for internal debugging */
    switch (g->gckind) {
    ...
    vmki_tracegc(L, 0);  /* for internal debugging */
  }
}
...
*/
static void fullinc (vmk_State *L, global_State *g) {
  if (keepinvariant(g))  /* black objects? */
    entersweep(L); /* sweep everything to turn them back to white */
  /* finish any pending sweep phase to start a new cycle */
  vmkC_runtilstate(L, GCSpause, 1);
  vmkC_runtilstate(L, GCScallfin, 1);  /* run up to finalizers */
  vmkC_runtilstate(L, GCSpause, 1);  /* finish collection */
  setpause(g);
}
...
*/
void vmkC_fullgc (vmk_State *L, int isemergency) {
  global_State *g = G(L);
  vmk_assert(!g->gcemergency);
