...
** it fits in a 'size_t'.
*/
#define MAXHSIZE	vmkM_limitN(1 << MAXHBITS, Node)
...
static const Node dummynode_ = {
  {{NULL}, VMK_VEMPTY,  /* value's value and type */
   VMK_TDEADKEY, 0, {NULL}}  /* key type, next, and key value */
};
...
*/
static Node *hashint (const Table *t, vmk_Integer i) {
  vmk_Unsigned ui = l_castS2U(i);
...
static unsigned l_hashfloat (vmk_Number n) {
  int i;
  vmk_Integer ni;
  n = l_mathop(frexp)(n, &i) * -cast_num(INT_MIN);
  if (!vmk_numbertointeger(n, &ni)) {  /* is 'n' inf/-inf/NaN? */
    vmk_assert(vmki_numisnan(n) || l_mathop(fabs)(n) == cast_num(HUGE_VAL));
    return 0;
  }
  else {  /* normal case */
...
static Node *mainpositionTV (const Table *t, const TValue *key) {
  switch (ttypetag(key)) {
    case VMK_VNUMINT: {
      vmk_Integer i = ivalue(key);
      return hashint(t, i);
    }
    case VMK_VNUMFLT: {
      vmk_Number n = fltvalue(key);
    ...
    case VMK_VLNGSTR: {
      TString *ts = tsvalue(key);
      return hashpow2(t, vmkS_hashlongstr(ts));
    }
    case VMK_VFALSE:
      return hashboolean(t, 0);
    case VMK_VTRUE:
      return hashboolean(t, 1);
    case VMK_VLIGHTUSERDATA: {
      void *p = pvalue(key);
      return hashpointer(t, p);
    }
    case VMK_VLCF: {
      vmk_CFunction f = fvalue(key);
...
l_sinline Node *mainpositionfromnode (const Table *t, Node *nd) {
  TValue key;
  getnodekey(cast(vmk_State *, NULL), &key, nd);
...
static int equalkey (const TValue *k1, const Node *n2, int deadok) {
  ...
  switch (keytt(n2)) {
    case VMK_VNIL: case VMK_VFALSE: case VMK_VTRUE:
      return 1;
    case VMK_VNUMINT:
      return (ivalue(k1) == keyival(n2));
    case VMK_VNUMFLT:
      return vmki_numeq(fltvalue(k1), fltvalueraw(keyval(n2)));
    case VMK_VLIGHTUSERDATA:
      return pvalue(k1) == pvalueraw(keyval(n2));
    case VMK_VLCF:
      return fvalue(k1) == fvalueraw(keyval(n2));
    case ctb(VMK_VLNGSTR):
      return vmkS_eqlngstr(tsvalue(k1), keystrval(n2));
    default:
...
** the range [1, limit].
*/
static unsigned checkrange (vmk_Integer k, unsigned limit) {
...
*/
static unsigned findindex (vmk_State *L, Table *t, TValue *key,
                               unsigned asize) {
  unsigned int i;
  ...
  if (i != 0)  /* is 'key' inside array part? */
    ...
    if (l_unlikely(isabstkey(n)))
      vmkG_runerror(L, "invalid key to 'next'");  /* key not found */
...
int vmkH_next (vmk_State *L, Table *t, StkId key) {
...
** entry: Two values plus 'next' versus one value.) Evavmkte with size_t
** to avoid overflows.
*/
...
static void countint (vmk_Integer key, Counters *ct) {
  unsigned int k = arrayindex(key);
  if (k != 0) {  /* is 'key' an array index? */
    ct->nums[vmkO_ceillog2(k)]++;  /* count as such */
    ct->na++;
  }
}
...
static void numusehash (const Table *t, Counters *ct) {
  ...
  while (i--) {
    Node *n = &t->node[i];
    if (isempty(gval(n))) {
      vmk_assert(!keyisnil(n));  /* entry was deleted; key cannot be nil */
      ct->deleted = 1;
    }
    else {
      total++;
...
*/
static Value *resizearray (vmk_State *L , Table *t,
                               unsigned oldasize,
                               unsigned newasize) {
  if (oldasize == newasize)
    ...
    vmkM_freemem(L, op, concretesize(oldasize));  /* free it */
    return NULL;
  }
  else {
    size_t newasizeb = concretesize(newasize);
    Value *np = cast(Value *,
                  vmkM_reallocvector(L, NULL, 0, newasizeb, lu_byte));
    ...
    if (oldasize > 0) {
      ...
      vmk_assert(tomoveb > 0);
      memcpy(np - tomove, op - tomove, tomoveb);
      vmkM_freemem(L, op - oldasize, oldasizeb);  /* free old block */
    }
    return np;
  }
}
...
** overflow.
*/
static void setnodevector (vmk_State *L, Table *t, unsigned size) {
  if (size == 0) {  /* no elements to hash part? */
    ...
    int i;
    int lsize = vmkO_ceillog2(size);
    if (lsize > MAXHBITS || (1 << lsize) > MAXHSIZE)
      vmkG_runerror(L, "table overflow");
    size = twoto(lsize);
    if (lsize < LIMFORLAST)  /* no 'lastfree' field? */
      t->node = vmkM_newvector(L, size, Node);
    else {
      size_t bsize = size * sizeof(Node) + sizeof(Limbox);
      char *node = vmkM_newblock(L, bsize);
...
*/
static void reinserthash (vmk_State *L, Table *ot, Table *t) {
  unsigned j;
...
static void clearNewSlice (Table *t, unsigned oldasize, unsigned newasize) {
  for (; oldasize < newasize; oldasize++)
    *getArrTag(t, oldasize) = VMK_VEMPTY;
}
...
** part.
*/
void vmkH_resize (vmk_State *L, Table *t, unsigned newasize,
                                          unsigned nhsize) {
  ...
  if (l_unlikely(newarray == NULL && newasize > 0)) {  /* allocation failed? */
    ...
    vmkM_error(L);  /* raise error (with array unchanged) */
  }
...
void vmkH_resizearray (vmk_State *L, Table *t, unsigned int nasize) {
  unsigned nsize = allocsizenode(t);
  vmkH_resize(L, t, nasize, nsize);
}
...
*/
static void rehash (vmk_State *L, Table *t, const TValue *ek) {
...
Table *vmkH_new (vmk_State *L) {
  GCObject *o = vmkC_newobj(L, VMK_VTABLE, sizeof(Table));
  Table *t = gco2t(o);
  t->metatable = NULL;
...
** Frees a table.
*/
void vmkH_free (vmk_State *L, Table *t) {
  freehash(L, t);
...
static int insertkey (Table *t, const TValue *key, TValue *value) {
  ...
  vmk_assert(isabstkey(getgeneric(t, key, 0)));
  if (!isempty(gval(mp)) || isdummy(t)) {  /* main position is taken? */
    Node *othern;
    Node *f = getfreepos(t);  /* get a free place */
    if (f == NULL)  /* cannot find a free place? */
      return 0;
    vmk_assert(!isdummy(t));
    ...
    if (othern != mp) {  /* is colliding node out of its main position? */
      ...
      *f = *mp;  /* copy colliding node into free pos. (mp->next also goes) */
      ...
      else vmk_assert(gnext(f) == 0);
      gnext(mp) = cast_int(f - mp);
      mp = f;
    }
  }
  setnodekey(mp, key);
  vmk_assert(isempty(gval(mp)));
  setobj2t(cast(vmk_State *, 0), gval(mp), value);
  return 1;
}
...
static void newcheckedkey (Table *t, const TValue *key, TValue *value) {
  ...
  if (i > 0)  /* is key in the array part? */
    ...
    vmk_assert(done);  /* it cannot fail */
    cast(void, done);  /* to avoid warnings */
  }
}


static void vmkH_newkey (vmk_State *L, Table *t, const TValue *key,
                                                 TValue *value) {
  if (!ttisnil(value)) {  /* do not insert nil values */
    ...
    vmkC_barrierback(L, obj2gco(t), key);
...
static const TValue *getintfromhash (Table *t, vmk_Integer key) {
  Node *n = hashint(t, key);
  vmk_assert(!ikeyinarray(t, key));
  for (;;) {  /* check whether 'key' is somewhere in the chain */
...
static int hashkeyisempty (Table *t, vmk_Unsigned key) {
...
static lu_byte finishnodeget (const TValue *val, TValue *res) {
  if (!ttisnil(val)) {
    setobj(((vmk_State*)NULL), res, val);
  }
  return ttypetag(val);
}


lu_byte vmkH_getint (Table *t, vmk_Integer key, TValue *res) {
...
*/
const TValue *vmkH_Hgetshortstr (Table *t, TString *key) {
  Node *n = hashstr(t, key);
  vmk_assert(strisshr(key));
  for (;;) {  /* check whether 'key' is somewhere in the chain */
...
lu_byte vmkH_getshortstr (Table *t, TString *key, TValue *res) {
  return finishnodeget(vmkH_Hgetshortstr(t, key), res);
}
...
static const TValue *Hgetlongstr (Table *t, TString *key) {
  TValue ko;
  vmk_assert(!strisshr(key));
  setsvalue(cast(vmk_State *, NULL), &ko, key);
...
static const TValue *Hgetstr (Table *t, TString *key) {
  if (strisshr(key))
    return vmkH_Hgetshortstr(t, key);
  else
    return Hgetlongstr(t, key);
}


lu_byte vmkH_getstr (Table *t, TString *key, TValue *res) {
...
** main search fn
*/
lu_byte vmkH_get (Table *t, const TValue *key, TValue *res) {
  const TValue *slot;
  switch (ttypetag(key)) {
    case VMK_VSHRSTR:
      slot = vmkH_Hgetshortstr(t, tsvalue(key));
      break;
    case VMK_VNUMINT:
      return vmkH_getint(t, ivalue(key), res);
    case VMK_VNIL:
      slot = &absentkey;
      break;
    case VMK_VNUMFLT: {
      vmk_Integer k;
      if (vmkV_flttointeger(fltvalue(key), &k, F2Ieq)) /* integral index? */
        return vmkH_getint(t, k, res);  /* use specialized version */
      /* else... */
    }  /* FALLTHROUGH */
    default:
...
/*
** When a 'pset' cannot be completed, this fn returns an encoding
** of its result, to be used by 'vmkH_finishset'.
*/
...
static int finishnodeset (Table *t, const TValue *slot, TValue *val) {
  if (!ttisnil(slot)) {
    setobj(((vmk_State*)NULL), cast(TValue*, slot), val);
    return HOK;  /* success */
  }
  else
...
int vmkH_psetint (Table *t, vmk_Integer key, TValue *val) {
  vmk_assert(!ikeyinarray(t, key));
  return finishnodeset(t, getintfromhash(t, key), val);
}


static int psetint (Table *t, vmk_Integer key, TValue *val) {
  int hres;
  vmkH_fastseti(t, key, val, hres);
  return hres;
}


/*
** This fn could be just this:
**    return finishnodeset(t, vmkH_Hgetshortstr(t, key), val);
...
int vmkH_psetshortstr (Table *t, TString *key, TValue *val) {
  const TValue *slot = vmkH_Hgetshortstr(t, key);
  if (!ttisnil(slot)) {  /* key already has a value? (all too common) */
    setobj(((vmk_State*)NULL), cast(TValue*, slot), val);  /* update it */
    return HOK;  /* done */
  }
    ...
    if (isabstkey(slot) &&  /* key is absent? */
      ...
      setsvalue(cast(vmk_State *, NULL), &tk, key);
...
int vmkH_psetstr (Table *t, TString *key, TValue *val) {
  if (strisshr(key))
    return vmkH_psetshortstr(t, key, val);
  else
    return finishnodeset(t, Hgetlongstr(t, key), val);
}


int vmkH_pset (Table *t, const TValue *key, TValue *val) {
  switch (ttypetag(key)) {
    case VMK_VSHRSTR: return vmkH_psetshortstr(t, tsvalue(key), val);
    case VMK_VNUMINT: return psetint(t, ivalue(key), val);
    case VMK_VNIL: return HNOTFOUND;
    case VMK_VNUMFLT: {
      vmk_Integer k;
      if (vmkV_flttointeger(fltvalue(key), &k, F2Ieq)) /* integral index? */
...
void vmkH_finishset (vmk_State *L, Table *t, const TValue *key,
  ...
  if (hres == HNOTFOUND) {
    ...
    if (l_unlikely(ttisnil(key)))
      vmkG_runerror(L, "table index is nil");
    else if (ttisfloat(key)) {
      vmk_Number f = fltvalue(key);
      vmk_Integer k;
      if (vmkV_flttointeger(f, &k, F2Ieq)) {
        setivalue(&aux, k);  /* key is equal to an integer */
        key = &aux;  /* insert it as an integer */
      }
      else if (l_unlikely(vmki_numisnan(f)))
        vmkG_runerror(L, "table index is NaN");
    }
    vmkH_newkey(L, t, key, value);
  }
...
void vmkH_set (vmk_State *L, Table *t, const TValue *key, TValue *value) {
  int hres = vmkH_pset(t, key, value);
  if (hres != HOK)
    vmkH_finishset(L, t, key, value, hres);
}
...
*/
void vmkH_setint (vmk_State *L, Table *t, vmk_Integer key, TValue *value) {
...
** If the doubling would overflow, we try VMK_MAXINTEGER. If it is
** absent, we are ready for the binary search. ('j', being max integer,
** is larger or equal to 'i', but it cannot be equal because it is
** absent while 'i' is present; so 'j > i'.) Otherwise, 'j' is a
** boundary. ('j + 1' cannot be a present integer key because it is
** not a valid integer in Vmk.)
*/
static vmk_Unsigned hash_search (Table *t, vmk_Unsigned j) {
  vmk_Unsigned i;
  if (j == 0) j++;  /* the caller ensures 'j + 1' is present */
  do {
    i = j;  /* 'i' is a present index */
    if (j <= l_castS2U(VMK_MAXINTEGER) / 2)
      j *= 2;
    else {
      j = VMK_MAXINTEGER;
...
static vmk_Unsigned newhint (Table *t, unsigned hint) {
  vmk_assert(hint <= t->asize);
  *lenhint(t) = hint;
  return hint;
}
...
*/
vmk_Unsigned vmkH_getn (Table *t) {
  ...
  vmk_assert(asize == 0 || !arraykeyisempty(t, asize));
...
Node *vmkH_mainposition (const Table *t, const TValue *key) {
  return mainpositionTV(t, key);
}
