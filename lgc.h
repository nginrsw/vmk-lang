...
#define vmkC_white(g)	cast_byte((g)->currentwhite & WHITEBITS)
...
/*
** Minor collections will shift to major ones after VMKI_MINORMAJOR%
** bytes become old.
*/
#define VMKI_MINORMAJOR         70

/*
** Major collections will shift to minor ones after a collection
** collects at least VMKI_MAJORMINOR% of the new bytes.
*/
#define VMKI_MAJORMINOR         50

/*
** A young (minor) collection will run after creating VMKI_GENMINORMUL%
** new bytes.
*/
#define VMKI_GENMINORMUL         20


/* incremental */

/* Number of bytes must be VMKI_GCPAUSE% before starting new cycle */
#define VMKI_GCPAUSE    250

/*
** Step multiplier: The collector handles VMKI_GCMUL% work units for
** each new allocated word. (Each "work unit" corresponds roughly to
** sweeping one object or traversing one slot.)
*/
#define VMKI_GCMUL      200

/* How many bytes to allocate before next GC step */
#define VMKI_GCSTEPSIZE	(200 * sizeof(Table))


#define setgcparam(g,p,v)  (g->gcparams[VMK_GCP##p] = vmkO_codeparam(v))
#define applygcparam(g,p,x)  vmkO_applyparam(g->gcparams[VMK_GCP##p], x)
...
#define GCSTPCLS	4  /* bit true when closing Vmk state */
...
#define condchangemem(L,pre,pos,emg)  \
	{ if (gcrunning(G(L))) { pre; vmkC_fullgc(L, emg); pos; } }
#endif

#define vmkC_condGC(L,pre,pos) \
	{ if (G(L)->GCdebt <= 0) { pre; vmkC_step(L); pos;}; \
	  condchangemem(L,pre,pos,0); }

/* more often than not, 'pre'/'pos' are empty */
#define vmkC_checkGC(L)		vmkC_condGC(L,(void)0,(void)0)


#define vmkC_objbarrier(L,p,o) (  \
	(isblack(p) && iswhite(o)) ? \
	vmkC_barrier_(L,obj2gco(p),obj2gco(o)) : cast_void(0))

#define vmkC_barrier(L,p,v) (  \
	iscollectable(v) ? vmkC_objbarrier(L,p,gcvalue(v)) : cast_void(0))

#define vmkC_objbarrierback(L,p,o) (  \
	(isblack(p) && iswhite(o)) ? vmkC_barrierback_(L,p) : cast_void(0))

#define vmkC_barrierback(L,p,v) (  \
	iscollectable(v) ? vmkC_objbarrierback(L, p, gcvalue(v)) : cast_void(0))

VMKI_FUNC void vmkC_fix (vmk_State *L, GCObject *o);
VMKI_FUNC void vmkC_freeallobjects (vmk_State *L);
VMKI_FUNC void vmkC_step (vmk_State *L);
VMKI_FUNC void vmkC_runtilstate (vmk_State *L, int state, int fast);
VMKI_FUNC void vmkC_fullgc (vmk_State *L, int isemergency);
VMKI_FUNC GCObject *vmkC_newobj (vmk_State *L, lu_byte tt, size_t sz);
VMKI_FUNC GCObject *vmkC_newobjdt (vmk_State *L, lu_byte tt, size_t sz,
                                                 size_t offset);
VMKI_FUNC void vmkC_barrier_ (vmk_State *L, GCObject *o, GCObject *v);
VMKI_FUNC void vmkC_barrierback_ (vmk_State *L, GCObject *o);
VMKI_FUNC void vmkC_checkfinalizer (vmk_State *L, GCObject *o, Table *mt);
VMKI_FUNC void vmkC_changemode (vmk_State *L, int newmode);
