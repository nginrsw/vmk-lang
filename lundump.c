...
typedef struct {
  ...
  vmk_Integer nstr;  /* number of strings in the list */
...
static void loadAlign (LoadState *S, unsigned align) {
  ...
  if (padding < align) {  /* (padding == align) means no padding */
    vmk_Integer paddingContent;
    loadBlock(S, &paddingContent, padding);
    vmk_assert(S->offset % align == 0);
  }
}
...
static const void *getaddr_ (LoadState *S, size_t size) {
  const void *block = vmkZ_getaddr(S->Z, size);
  S->offset += size;
  if (block == NULL)
...
static void loadString (LoadState *S, Proto *p, TString **sl) {
  ...
  if (size == 0) {  /* no string? */
    vmk_assert(*sl == NULL);  /* must be prefilled */
    return;
  }
  else if (size == 1) {  /* previously saved string? */
    vmk_Integer idx = cast(vmk_Integer, loadSize(S));  /* get its index */
    TValue stv;
    vmkH_getint(S->h, idx, &stv);  /* get its value */
    *sl = ts = tsvalue(&stv);
    vmkC_objbarrier(L, p, ts);
    return;  /* do not save it again */
  }
  else if ((size -= 2) <= VMKI_MAXSHORTLEN) {  /* short string? */
    char buff[VMKI_MAXSHORTLEN + 1];  /* extra space for '\0' */
    loadVector(S, buff, size + 1);  /* load string into buffer */
    *sl = ts = vmkS_newlstr(L, buff, size);  /* create string */
    vmkC_objbarrier(L, p, ts);
  }
  else if (S->fixed) {  /* for a fixed buffer, use a fixed string */
    const char *s = getaddr(S, size + 1, char);  /* get content address */
    *sl = ts = vmkS_newextlstr(L, s, size, NULL, NULL);
    vmkC_objbarrier(L, p, ts);
  }
  else {  /* create internal copy */
    *sl = ts = vmkS_createlngstrobj(L, size);  /* create string */
    vmkC_objbarrier(L, p, ts);
  ...
  S->nstr++;
  setsvalue(L, &sv, ts);
  vmkH_setint(L, S->h, S->nstr, &sv);
  vmkC_objbarrierback(L, obj2gco(S->h), ts);
}
...
*/
LClosure *vmkU_undump (vmk_State *L, ZIO *Z, const char *name, int fixed) {
  LoadState S;
  LClosure *cl;
  if (*name == '@' || *name == '=')
    S.name = name + 1;
  ...
  vmkD_inctop(L);
  S.h = vmkH_new(L);  /* create list of saved strings */
  S.nstr = 0;
