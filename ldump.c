...
typedef struct {
  ...
  vmk_Integer nstr;  /* counter for counting saved strings */
} DumpState;
...
static void dumpAlign (DumpState *D, unsigned align) {
  ...
  if (padding < align) {  /* padding == align means no padding */
    static vmk_Integer paddingContent = 0;
    vmk_assert(align <= sizeof(vmk_Integer));
    dumpBlock(D, &paddingContent, padding);
  }
  vmk_assert(D->offset % align == 0);
}
...
static void dumpNumber (DumpState *D, vmk_Number x) {
  dumpVar(D, x);
}


static void dumpInteger (DumpState *D, vmk_Integer x) {
  dumpVar(D, x);
}
...
static void dumpString (DumpState *D, TString *ts) {
  if (ts == NULL)
    ...
    TValue idx;
    int tag = vmkH_getstr(D->h, ts, &idx);
    if (!tagisempty(tag)) {  /* string already saved? */
      ...
      vmkH_set(D->L, D->h, &key, &value);  /* h[ts] = nstr */
  ...
  for (i = 0; i < n; i++) {
    ...
    switch (tt) {
      ...
      case VMK_VLNGSTR:
        ...
        break;
      default:
        vmk_assert(tt == VMK_VNIL || tt == VMK_VFALSE || tt == VMK_VTRUE);
    }
  }
}
...
static void dumpHeader (DumpState *D) {
  dumpLiteral(D, VMK_SIGNATURE);
  dumpByte(D, VMKC_VERSION);
  dumpByte(D, VMKC_FORMAT);
  dumpLiteral(D, VMKC_DATA);
  dumpByte(D, sizeof(Instruction));
  dumpByte(D, sizeof(vmk_Integer));
  dumpByte(D, sizeof(vmk_Number));
  dumpInteger(D, VMKC_INT);
  dumpNumber(D, VMKC_NUM);
}
...
int vmkU_dump (vmk_State *L, const Proto *f, vmk_Writer w, void *data,
               int strip) {
  DumpState D;
  D.h = vmkH_new(L);  /* aux. table to keep strings already dumped */
